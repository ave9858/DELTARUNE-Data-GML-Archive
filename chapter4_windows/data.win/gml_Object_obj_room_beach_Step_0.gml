if (canrest) {
	if (obj_mainchara.x > x && (!sunkus_kb_check_pressed(vk_anykey) && !scr_gamepad_check_any()) && global.interact == 0)
		timer++;
	else if (timer > 0)
		timer = 0;
	...
	if (global.chapter == 2) {
		...
		if (con == 3 && customcon == 1 && (sunkus_kb_check_pressed(vk_anykey) || scr_gamepad_check_any())) {
			con = 4;
			customcon = 0;
			alarm[0] = 30;
			c_waitcustom_end();
