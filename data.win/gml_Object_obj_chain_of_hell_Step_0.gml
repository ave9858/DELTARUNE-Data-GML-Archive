...
if (chaincon == 1) {
	mypath1 = path_hellchain_hor1;
	mypath2 = path_hellchain_hor2;

	if (type == 1) {
		mypath1 = path_hellchain_a;
		mypath2 = path_hellchain_b;
		factor = 8;
	}

	path_start(mypath1, maxspeed, path_action_stop, 0);
	chaincon = 2;
	chaintimer = 0;
}
...
if (smashtime == 1) {
	if (type == 0) {
		...
		if (btimer == 10) {
			for (i = 0; i < 1; i += 1) {
				regbul = instance_create(__view_get(0, 0) + 320 + (i * 120), __view_get(1, 0) - 20, obj_regularbullet);
				regbul.vspeed = 2;
				regbul.gravity = 0.2;
		...
		if (btimer >= 20) {
			...
			for (i = 0; i < 2; i += 1) {
				regbul = instance_create(__view_get(0, 0) + 260 + (i * 120), __view_get(1, 0) - 20, obj_regularbullet);
				regbul.vspeed = 2;
				regbul.gravity = 0.2;
				regbul.sprite_index = spr_spadebullet;
				regbul.image_angle = 270;
