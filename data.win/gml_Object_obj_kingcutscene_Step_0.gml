if (con < 76) {
	...
	if (con == 53) {
		...
		lspade = scr_marker(__view_get(0, 0) + 660, king.y + 60, spr_spadebullet);
...
if (con >= 76 && con < 150) {
	...
	if (con == 145) {
		if (__view_get(0, 0) > 0) {
			__view_set(0, 0, __view_get(0, 0) + (k.hspeed * 2));
...
if (con >= 150 && con < 250) {
	...
	if (con == 206 && !d_ex()) {
		r = scr_dark_marker(__view_get(0, 0) + 640, s.y + 10, spr_ralseil);
	...
	if (con == 216 && !d_ex()) {
		...
		l = scr_dark_marker(__view_get(0, 0) - 80, k.y, spr_lancer_rt);
...
if (con >= 250 && con <= 265) {
	if (con == 250) {
		...
		for (i = 0; i < xmake; i += 1) {
			for (j = 0; j < ymake; j += 1) {
				cwp[i, j] = scr_dark_marker(__view_get(0, 0) - 180 - (i * 90), 230 + (j * 30), spr_crowdpiece);
		...
		l = scr_dark_marker(__view_get(0, 0) - 180 - 280, 60, spr_lancer_rt);
...
if (con >= 350 && con < 700) {
	...
	if (con == 353 && !d_ex()) {
		...
		scr_pan(3, 0, 60);
		r = scr_dark_marker(__view_get(0, 0) + 800, s.y + 10, spr_ralseil);
